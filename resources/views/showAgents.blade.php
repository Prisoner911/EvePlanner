                ...
                <table class="">
                    <thead>
                        ...
                        <tr style="">
                            ...
                            <th scope="col" style="text-align: center">Access Rights to customers</th>
                        </tr>
                    </thead>
                            ...
                            @foreach ($agents as $agent)
                                <tr class="t_head">
                                    ...
                                    <td scope="row">
                                        ...
                                        <a href="{{ route('agent.edit', ['id' => $agent->AgentID]) }}"><button
                                                class="btn btn-primary">Edit
                                                Details</button></a>
                                        <a href="{{ route('agent.delete', ['id' => $agent->AgentID]) }}"><button
                                                class="btn btn-danger" style="font-size: 14px; margin-top: 2px;">Remove
                                                Agent</button></a><br>
                                        <a href="{{ route('viewEventsAssigned', ['id' => $agent->AgentID]) }}"><button
                                                class="btn btn-info mt-2">Show Assigned
                                                Events</button></a>
                                    ...
                                    <td>
                                        ...
                                        <form action="/managerAccess/Agents/{{ $agent->AgentID }}" method="POST">

                                            @csrf
                                            <div class="mainDiv">
                                                <div class="radioDiv">

                                                    <input type="radio" name="agentStatus" class="form-check-input"
                                                        id="option1_{{ $agent->AgentID }}" checked value="no">
                                                    <label for="option1_{{ $agent->AgentID }}">No</label>

                                                    <input type="radio" name="agentStatus" class="form-check-input mb-3"
                                                        id="option2_{{ $agent->AgentID }}" value="yes"
                                                        {{ $agent->Access_Rights == 'yes' ? 'checked' : '' }}>
                                                    <label for="option2_{{ $agent->AgentID }}">Yes</label>
                                                </div>

                                                <a href="{{ route('AgentStatus', ['id' => $agent->AgentID]) }}"><button
                            ...
                            if (agentSearch.length > 0) {
                                for (let i = 0; i < agentSearch.length; i++) {
                                    ...
                                    html += '<tr class="t_head">' +
                                        ...
                                        '</a>' + '<br>' +
                                        ' <a href="/managerAccess/Agents/viewAssignedEvents/"' +
                                        agentSearch[i]
                                        ['AgentID'] + '">' +
                                        '<button class="btn btn-info mt-2">Show Assigned Events</button>' +
                                        '</a>' +
                                        '</td>' +
                                        '<td scope="row">' +
                                        ...
                                        '<div class="mainDiv">' +
                                        '<div class="radioDiv">' +
                                        '<input type="radio" name="agentStatus" class="form-check-input" id="option1_' +
                                        agentSearch[i]
                                        ['AgentID'] + '" checked value="no">' +
                                        '<label for="option1_' +
                                        agentSearch[i]
                                        ['AgentID'] + '" style="margin-left: 5px;">No</label>' +
                                        '<input type="radio" name="agentStatus" style="margin-left: 5px;" class="form-check-input mb-3" id="option2_' +
                                        agentSearch[i]
                                        ['AgentID'] + '" value="yes" ' +
